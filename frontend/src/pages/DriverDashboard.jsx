...
export default function DriverDashboard() {
  ...
  const loadRides = async (isBackground = false) => {
    try {
      if (!isBackground) console.log("=== LOADING RIDES ===");
      ...
      try {
        console.log("Fetching available rides and vacations...");
        ...
        // Filter myVacations to only include assigned vacations (confirmed/in_progress/completed)
        // The backend already filters this, but good to be safe or if we want to filter out cancelled locally
        const assignedVacations = Array.isArray(myVacationsData) ? myVacationsData : [];

        // Only update if we got valid arrays (even if empty)
        if (Array.isArray(localAvailable)) setLocalRides(localAvailable);
        if (Array.isArray(vacationAvailable)) setVacationRides(vacationAvailable);
        if (Array.isArray(assignedRides)) setMyRides(assignedRides);
        if (Array.isArray(assignedVacations)) setMyVacations(assignedVacations);
    ...
    const connectWebSocket = () => {
      ...
      if (!token) {
        ...
        if (reconnectAttempts < maxReconnectAttempts) {
          ...
          alert('Connection failed. Please refresh the page to receive ride requests.');
        }
        return;
      }
      ...
      ws.onclose = () => {
        ...
        if (reconnectAttempts < maxReconnectAttempts) {
          ...
          console.error('Max reconnect attempts reached. Please refresh the page.');
          alert('Connection lost. Please refresh the page to receive ride requests.');
        }
      };
    };
